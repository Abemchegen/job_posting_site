...
public class ChatController {
    ...
    @GetMapping("/contacts")
    public ResponseEntity<List<UserResponse>> getChatContacts(@AuthenticationPrincipal User currentUser) {
        List<UserResponse> response = chatService.getChatContacts(currentUser.getId());
    ...
    public ResponseEntity<List<ChatMessage>> getChatHistory(@AuthenticationPrincipal User currentUser,
        ...
        List<ChatMessage> response = chatService.getChatHistory(currentUser.getId(), userid);
        return ResponseEntity.ok().body(response);
    }
